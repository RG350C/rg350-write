...
psp_fmgr_ask_confirm(void)
{
  ...
  psp_sdl_back_print(190,  80, "press B to confirm !", 
                     GP2X_MENU_WARNING_COLOR);
  psp_sdl_flip();
...
static char user_filedir_txt[GP2X_FMGR_MAX_NAME];
...
psp_fmgr_menu(int format)
{
  ...
  if (psp_file_request(user_filename, user_filedir)) {
    ...
    else 
    {
      ...
      if (file_format == GP2X_FMGR_FORMAT_TXT) /* Txt */ error = psp_editor_load(user_filename);
