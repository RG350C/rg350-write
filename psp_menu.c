...
psp_edit_menu_confirm_overwrite()
{
  ...
  if (PSPWRITE.ask_overwrite) {
    ...
    psp_sdl_back_print(150, 60, "B overwrite ? Y cancel", GP2X_MENU_WARNING_COLOR);
    psp_sdl_flip();
...
psp_edit_menu_confirm_save()
{
  ...
  if (PSPWRITE.is_modified) {
    ...
    psp_display_screen_menu();
    psp_sdl_back_print(150, 60, "Save changes ? B Save ", GP2X_MENU_WARNING_COLOR);
    psp_sdl_back_print(150, 70, "A ignore, Y cancel", GP2X_MENU_WARNING_COLOR);
    psp_sdl_flip();
...
psp_edit_menu_exit(void)
{
  ...
  psp_display_screen_menu();
  psp_sdl_back_print(150, 60, "Press B to exit !", GP2X_MENU_WARNING_COLOR);
  psp_sdl_flip();
