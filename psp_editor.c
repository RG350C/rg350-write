...
psp_editor_init()
{
  ...
  editor_colors[COLOR_IMAGE         ] = GP2X_MENU_TEXT_COLOR;

  psp_sdl_select_font( 0 );
}

# define GP2X_EDITOR_BLINK_TIME 500
...
psp_display_cursor()
{
  ...
  if ((curr_clock - g_text->last_blink) > GP2X_EDITOR_BLINK_TIME) {
...
# define GP2X_EDITOR_MIN_MOVE_TIME   10000
# define GP2X_EDITOR_REPEAT_TIME    300000
# define GP2X_EDITOR_MIN_EDIT_TIME  180000
# define GP2X_EDITOR_MIN_TIME       150000
void
psp_editor_main_loop()
{
  ...
  while (1) {
    ...
    if (danzeff_mode) {
      ...
      danzeff_render( PSPWRITE.transparent );
    }
    psp_sdl_flip();
...
# endif
      ...
      if (new_pad != 0) {
        if ((c.TimeStamp - last_time) > GP2X_EDITOR_REPEAT_TIME) {
          repeat_mode = 1;
        }
      } else {
        repeat_mode = 0;
      ...
      if (repeat_mode) {
        if ((c.TimeStamp - last_time) > GP2X_EDITOR_MIN_MOVE_TIME) {
          last_time = c.TimeStamp;
          old_pad = new_pad;
          break;
        }
        ...
        if ((c.TimeStamp - last_time) > GP2X_EDITOR_MIN_TIME) {
          last_time = c.TimeStamp;
          old_pad = new_pad;
          break;
        }
