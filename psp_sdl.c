...
{
# if defined(DINGUX_MODE)
  cacheflush((char *)screen_surface->pixels, screen_surface->w * screen_surface->h * sizeof(unsigned short), DCACHE);
# endif
}
# endif
...
{
  if ((x < 0) || ((x + w) > GP2X_SDL_SCREEN_WIDTH) || 
      (y < 0) || ((y + h) > GP2X_SDL_SCREEN_HEIGHT)) return;
